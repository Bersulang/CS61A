...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # END Problem 3 and 4
        tem_place=self.place
        count = 0
        while count < self.lower_bound:
            if tem_place.is_hive:
                return None
            tem_place=tem_place.entrance
            count+=1
        while count <= self.upper_bound:
            if tem_place.is_hive:
                return None
            if tem_place.bees:
                return random_bee(tem_place.bees)
            tem_place=tem_place.entrance
            count+=1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for i in self.place.bees[:]:
            i.reduce_health(reflect_damage)
...
# BEGIN Problem 6
class WallAnt(Ant):
    '''build an ant have a huge health but do nothing on each turn'''
    name = 'Wall'
    food_cost = 4
    implemented = True
...
# BEGIN Problem 7
class HungryAnt(Ant):
    '''build an ant that can reduce a huge damage but have a time limit'''
    name = 'Hungry'
    food_cost = 4
    chew_cooldown = 3
    implemented = True
...
class TankAnt(ContainerAnt):
    '''build an ant that can protect other ant and also can attack bees'''
    name = 'Tank'
    food_cost = 6
    damage = 1
    implemented = True
        ...
        while tmp_place:
            if tmp_place.ant:
                if isinstance(tmp_place.ant, ContainerAnt):
                    contained_ant = tmp_place.ant.ant_contained
                    if contained_ant:
