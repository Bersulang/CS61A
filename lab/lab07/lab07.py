class Account:
    ...
    def time_to_retire(self, amount):
        ...
        assume_balance = self.balance
        need_year=0
        while assume_balance<amount:
            assume_balance *= (self.interest+1)
            need_year+=1
        return need_year
...
def duplicate_link(s, val):
    ...
    """
    find = s
    while find is not Link.empty:
        if find.first == val:
            insert_link = Link(val)
            insert_link.rest = find.rest
            find.rest = insert_link
            find = insert_link
        find = find.rest
